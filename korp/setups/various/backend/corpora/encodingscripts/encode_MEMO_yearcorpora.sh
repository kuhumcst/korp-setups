...
if [ -d "$CORPORADIR/annotated/Brill-all/yearcorpora" ]
then
	...
	for f in $CORPORADIR/annotated/Brill-all/yearcorpora/*.xml; do
		...
		cwb-makeall -V -r $CORPORADIR/registry $CORPUSUPPER &>> memo_yearcorpora_encoding_output.txt

	    echo ""

	    PYTHONIOENCODING=utf-8 python3 $CORPORADIR/encodingscripts/create_MEMOconfigs.py $CORPUSNAME >> memo_yearcorpora_configs.txt
